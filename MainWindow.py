...
class MainWindow:
    def __init__(self, application):
        ...
        # Import UI file:
        self.builder.add_from_file(os.path.dirname(os.path.abspath(__file__)) + "/MainWindow.glade")
    ...
    def defineComponents(self):
        ...
        self.menu_aboutturan = UI("menu_aboutturan")
        ...
        self.dialog_about.set_program_name(_("Turan My Computer"))
        # Set version
    ...
    def add_to_desktop(self):
        ...
        user_desktopcontrol_file = os.path.join(user_home, ".config/turan-mycomputer/desktop")
        if not os.path.isfile(user_desktopcontrol_file):
            print("{} {}".format("Desktop file copying to",
                                 GLib.get_user_special_dir(GLib.UserDirectory.DIRECTORY_DESKTOP)))
            try:
                subprocess.Popen(["/bin/bash",
                                  "/usr/share/turan/proqramlar/autostart/turan-mycomputer-add-to-desktop"])
    ...
    def set_os_label_img(self):
        ...
        if os_id == "turan":
            self.img_os.set_from_icon_name("emblem-turan", Gtk.IconSize.BUTTON)
            self.img_os_menu.set_from_icon_name("emblem-turan", Gtk.IconSize.BUTTON)
            self.lbl_os_menu.set_label("{}".format(_("About Turan")))
        else:
            try:
    ...
    def get_controlpanel_desktops(self):
        blacklist = ["synaptic.desktop", "aze.turan.mycomputer.desktop"]
        apps = []
    ...
    def set_places(self):
        ...
        for loc in locs:
            ...
            listbox.get_style_context().add_class("turan-mycomputer-listbox")
        ...
        iconview.get_style_context().add_class("turan-mycomputer-iconview")
            ...
            if file_info is not None:
                ...
                try:
                    if file_info["usage_percent"] >= 0.9:
                        row_volume._pb_volume_size.get_style_context().add_class("turan-mycomputer-progress-90")
                except Exception as e:
    ...
    def addDisksToGUI(self):
        ...
        try:
            if root_info["usage_percent"] >= 0.9:
                self.pb_root_usage.get_style_context().add_class("turan-mycomputer-progress-90")
        except Exception as e:
            print("progress css exception: {}".format(e))
    ...
    def on_btn_format_removable_clicked(self, button):
        ...
        subprocess.Popen(["turan-usb-formatter", file_info["device"]])
    ...
    def on_menu_aboutturan_clicked(self, button):
        self.popover_menu.popdown()

        desktop = self.get_current_desktop()

        try:
            subprocess.Popen(["turan-about"])
        except Exception as e:
            print("{}".format(e))
            if "xfce" in desktop:
                try:
    ...
    def notify(self, message_summary="", message_body="", icon="computer"):
        try:
